	...
	for (auto s = 0; s < cloud_base_shape_texture_size; s++)
	{
		...
		for (auto t = 0; t < cloud_base_shape_texture_size; t++)
		{
			for (auto r = 0; r < cloud_base_shape_texture_size; r++)
			{
				...
				cloud_base_shape_texels[addr + 1] = unsigned char(255.0f * (1.0F - worley_fbm0));
				cloud_base_shape_texels[addr + 2] = unsigned char(255.0f * (1.0F - worley_fbm1));
				cloud_base_shape_texels[addr + 3] = unsigned char(255.0f * (1.0F - worley_fbm2));
		...
		for (auto t = 0; t < cloud_erosion_texture_size; t++)
		{
			for (auto r = 0; r < cloud_erosion_texture_size; r++)
			{
				...
				auto addr = r * cloud_erosion_texture_size * cloud_erosion_texture_size + t * cloud_erosion_texture_size
					+ s;
				addr *= 4;
				cloud_erosion_texels[addr] = unsigned char(255.0f * (1 - worley_fbm0));
				cloud_erosion_texels[addr + 1] = unsigned char(255.0f * (1 - worley_fbm1));
				cloud_erosion_texels[addr + 2] = unsigned char(255.0f * (1 - worley_fbm2));
...
void generate_perlin_noise()
{
	...
	for (auto s = 0; s < cloud_base_shape_texture_size; s++)
	{
		...
		for (auto t = 0; t < cloud_base_shape_texture_size; t++)
		{
			...
			const auto perlin_noiseb = std::clamp(pow(perlin(coord, frequency, octave_count), 1), 0.0F, 1.0F);
			const auto perlin_noiseg = std::clamp(pow(perlin(coord, frequency, octave_count), 2), 0.0F, 1.0F);
			const auto perlin_noiser = std::clamp(pow(perlin(coord, frequency, octave_count), 4), 0.0F, 1.0F);

			const auto perlin_noisea = std::clamp(1.5F * pow(perlin(coord, frequency, 1), 2), 0.0F, 1.0F);

			auto perlin_worley_noise_g = 0.0F;
			auto perlin_worley_noise_r = 0.0F;
			auto perlin_worley_noise_b = 0.0F;
			auto worley_fbm = 0.0F;
			{
				const float cell_count = 4;
				...
				worley_fbm = worley_noise0 * 0.5F + worley_noise1 * 0.25F + worley_noise2 * 0.125F + worley_noise3 *
					0.075F + worley_noise4 * 0.05F;
				...
				// matches figure 4.7 better
				// mapping perlin noise in between worley as minimum and 1.0 as maximum (as described in text of p.101 of GPU Pro 7)
				perlin_worley_noise_r = std::clamp(remap(1.0F - worley_fbm, 1.0F - perlin_noiser, 1.0, 0.0, 1.0), 0.0F,
					1.0F);
			...
			addr *= 4;
			cloud_base_shape_texels[addr] = unsigned char(255.0f * perlin_worley_noise_g);
			cloud_base_shape_texels[addr + 1] = unsigned char(255.0f * perlin_worley_noise_b);
			cloud_base_shape_texels[addr + 2] = unsigned char(255.0f * perlin_noisea);
