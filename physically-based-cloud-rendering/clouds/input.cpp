...
auto process_input(float dt, camera_t& camera) -> void
{
    ...
    if (buttons[GLFW_KEY_W]) { trans += forward * dt * camera.movement_speed; }

    if (buttons[GLFW_KEY_S]) { trans -= forward * dt * camera.movement_speed; }

    if (buttons[GLFW_KEY_D]) { trans += right * dt * camera.movement_speed; }

    if (buttons[GLFW_KEY_A]) { trans -= right * dt * camera.movement_speed; }

    if (buttons[GLFW_KEY_Q]) { trans += up * dt * camera.movement_speed; }

    if (buttons[GLFW_KEY_E]) { trans -= up * dt * camera.movement_speed; }
...
auto key_callback(GLFWwindow* window, int key, int /*scan_code*/, int action, int /*mode*/) -> void
{
    ...
    {
        if (!show_options) { glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_NORMAL); }
        else { glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED); }
        show_options = !show_options;
    }
    else if (action == GLFW_PRESS) { buttons[key] = true; }
    else if (action == GLFW_RELEASE) { buttons[key] = false; }
}
...
[[nodiscard]] auto options() -> bool { return show_options; }
